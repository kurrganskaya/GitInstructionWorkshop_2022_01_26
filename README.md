...
___
**Проект в *git* состоит из веток (*branches*).**
 * Ветка (*branch*) — это копия проекта, созданная для внесения отдельностоящих изменений. 

Также, обязательно существует основная защищенная ветка (`master`), в которую сливают изменения из других веток. Чаще всего, под определенную задачу выделяют отдельную ветку (в свою очередь, она может разделяться на другие ветки для подзадач), а после выполнения задачи и тестирования проекта с этими изменениям, выполняют слияние этой ветки с `master`-веткой.

Ветвление помогает работать над проектом в команде, редактировать код без страха потери информации. Ветки можно создавать, удалять, сливать, сравнивать и перемещаться между ними. О командах, необходимых для работы с ветками, поговорим подробнее в следующем разделе __Базовая работа с ветками в git__.
